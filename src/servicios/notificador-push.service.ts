...
export class NotificadorPushService {
  ...
  private readonly servidor: string = "https://elplatolisto-backend.onrender.com";
  constructor(private http: HttpClient, private dataUsuariosPush : DataUsuariosPushService) { }
  ...
  private notificarPorRol( mensaje: string, rol : TipoUsuario ) {
    this.http.post(this.servidor + "/notify-role",
      {
  ...
  private async notificar( mensaje: string, mail : string) {
    ...
    if(token){
      this.http.post(this.servidor + "/notify",
        {
          "title": "El Plato Listo",
          "body": mensaje,
          "token": token
  ...
  enviarMail(aceptacion: boolean, nombreUsuario: string, mail: string) {
    this.http.post(this.servidor + "/send-mail",
      {
